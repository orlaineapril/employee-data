...
def authenticate(employee_id, password):
    if employee_id in df['employee_'].values:
        stored_password = df.loc[df['employee_'] == employee_id, 'Password'].values[0]
...
def display_employee_data(employee_id):
    ...
    employee_data = df.loc[df['employee_'] == employee_id]
    for column in df.columns:
        if column != 'Password':
            new_value = st.text_input(f"{column}", employee_data[column].values[0])
            df.loc[df['employee_'] == employee_id, column] = new_value

# Function to display and edit all employee data (admin view)
def display_all_data():
    st.write("### Admin View: All Employee Data")
    for index, row in df.iterrows():
        st.write(f"#### employee_: {row['employee_']}")
        for column in df.columns:
            if column != 'Password':
                new_value = st.text_input(f"{column}", row[column], key=f"{row['employee_']}_{column}")
...
employee_id = st.sidebar.text_input("employee_")
...
if st.sidebar.button("Login"):
    if authenticate(employee_id, password):
        ...
        st.sidebar.error("Invalid employee_ or Password.")
